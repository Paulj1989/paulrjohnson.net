...
fit_mlms <- 
  function(data) {
    lmer(
      value ~ 1 + demean_squad_value + club_mean_value + time + 
        (1 + demean_squad_value | league/squad) + (1 | season),
      data = data, REML = TRUE,
...
models <-
  ...
  mutate(
    ...
    club_mean_value = mean(squad_value),
    demean_squad_value = squad_value - club_mean_value,
    .by = squad
  ) |> 
  tidyr::pivot_longer(
    cols = c(pts, xgd, gd),
...
cm <-
  c(
    "(Intercept)" = "(Intercept)",
    "demean_squad_value" = "Squad Value",
    "club_mean_value" = "Club Mean Value",
    "time" = "Time (Seasons)",
    "SD (Intercept squadleague)" = "_Club_: Intercept Variance",
    "SD (demean_squad_value squadleague)" = "_Club_: Slope Variance",
    "Cor (Intercept~demean_squad_value squadleague)" = 
      "_Club_: Intercept-Slope Covariance",
    "SD (Intercept league)" = "_League_: Intercept Variance",
    "SD (demean_squad_value league)" = "_League_: Slope Variance",
    "Cor (Intercept~demean_squad_value league)" = 
      "_League_: Intercept-Slope Covariance",
...
The intercept values are meaningless here because Club Mean Value has no meaningful zero. I would have liked to resolve this, but I'm not sure it's possible?

A 1% increase in squad value within a club leads to a ~0.15 increase in points in a season, a ~0.16 increase in xG difference, and a ~0.24 increase in goal difference.

A 1% increase in the club's average squad value (so the value of their squad, on average, over the entire 12-season period), is worth around ~0.2 additional points each season, a ~0.22 increase in xG difference, and a ~0.31 increase in goal difference.

<!-- Every 1% increase in squad value (which is an increase of ~€2.37m) on the mean squad value (~€237m) leads to a 0.19 increase in points in a season, 0.2 increase in xG difference, and 0.29 increase in goal difference. For further context, every 10% increase in squad value (which is an increase of ~€23.7m) leads to a 1.9 increase in points, a 2.0 increase in xG difference, and a 2.9 increase in goal difference. These differences are relatively small, but this is  if a €25m player can add around two points to a team's points total, that could be the difference between a relegation candidate staying up or not! -->
...
#       data,
#       terms = c("squad_value_within[n=100]", "squad_value_between[n=100]"),
...
#     se = FALSE, linewidth = 1.2, colour = "#343a40"
#     ) + 
#   geom_smooth(
...
models |>
  ...
  tidyr::unnest(preds) |> 
  mutate(
    squad_value = exp(demean_squad_value + club_mean_value),
  ...
  geom_smooth(
    aes(y = conf.low), method = lm, formula = y ~ log(x), se = FALSE, 
    linewidth = 0.6, colour = "#343a40", linetype = "dashed"
    ) + 
  geom_smooth(
    aes(y = conf.high), method = lm, formula = y ~ log(x), se = FALSE, 
    linewidth = 0.6, colour = "#343a40", linetype = "dashed"
...
The marginal gains for increases in squad value are the largest at the lower end of the values, appearing to start flattening out before the €250m point.
...
#     ggeffects::predict_response(
#       data,
#       terms = c("squad_value_within[n=100]", "squad_value_between[n=100]", "league"),
#       condition = c(season = "2023/24", time = 11),
#       type = "simulate"
#     )
#   }
...
#   #     summarise(
#   #       squad_value_gm = mean(squad_value),
#   #       .by = league
#   #       ),
#   #   by = join_by(group == league)
#   # ) |>
#   mutate(
#     group = as.numeric(as.character(group)),
#     x = exp(x + group),
...
#       ~ case_when(
#         facet %in% c("Bundesliga", "Ligue 1") ~ .x * 34,
#         facet %in% c("Premier League", "La Liga", "Serie A") ~ .x * 38
#         )
#       )
#     ) |> 
#   ggplot(aes(x, predicted, colour = facet)) +
...
league_preds <- 
  function(data) {
    predictions(
      data, type = "response",
      by = c("demean_squad_value", "club_mean_value", "league"),
      newdata = 
        datagrid(time = 11, season = "2023/24", grid_type = "counterfactual")
      )
    }
...
models |>
  ...
  summarise(estimate = mean(estimate), .by = c("league", "squad_value")) |> 
