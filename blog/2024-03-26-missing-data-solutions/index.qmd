...
If you know the data well and can identify precisely why the data is missing, you may also be able to plug in the missing values. However, you should be pretty sure when doing this because otherwise, you have personally taken the time to add some error to your data. This is bad for all but the most graft-thirsty High Performance Podcast listening maniacs. If you would take the [$500k over dinner with Jay-Z](https://i.kym-cdn.com/entries/icons/original/000/038/686/jayz.jpg), you should make sure your theory is sound!
...
missing_years <-
  ...
  mutate(
    total_working_years = replace(
      total_working_years,
      runif(n()) < 0.75 &
        relationship_satisfaction %in% c("High", "Very High"),
      NA
    ),
...
@rubin1987 devised formulas for pooling parameter estimates and standard errors, where the pooled parameter estimate is the arithmetic mean of the individual estimates, while pooled standard errors follow a similar logic but a slightly more complex process [@enders2022]. There are also methods for calculating statistical significance, but things are getting much more complicated now, and this is not the time or the place. I'd recommend reading "The Analysis and Pooling Phases of Multiple Imputation" in @enders2022's _Applied Missing Data Analysis_ if you want to get in the weeds.
  ...
  mutate(
    monthly_income = arm::rescale(
      replace(
        ...
        runif(n()) < 0.75 &
          (education %in%
            c("Bachelor", "Master", "Doctor") &
            job_role %in% c("Research Scientist", "Sales Executive")),
        NA
      )
    ),
    total_working_years = replace(
    ...
    if (m > 1) {
      data |>
        ...
        with(
          glm(
            attrition ~
              monthly_income + total_working_years + num_companies_worked,
            family = "binomial"
          )
        ) |>
        mice::pool()
...
original_glm <-
  ...
  glm(
    attrition ~ monthly_income + total_working_years + num_companies_worked,
    family = "binomial",
    data = _
  )
