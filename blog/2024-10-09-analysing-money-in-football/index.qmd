...
[^Time]: 
    ...
    I chose not to centre time because I don't think this would make the intercept easier to interpret. Consequently, all three models' intercepts are high because the 
    average club's average squad value will be high by 2012/13 standards.
...
[^Managers]:
    ...
    I have lumped manager effects in as a structural advantage on the assumption that better managers raise a club's baseline outcomes regardless of squad value. However, 
    manager effects may influence the club-level slope, too, as a better manager gets more out of investments in the squad. We also know that manager effects will bleed 
    into Transfermarkt's squad values - better managers improving players, leading to those players being given higher values - which will bias the population-level 
    estimates (especially the within-effect). This points to a limitation of the model and use of squad values. Football is a complex, dynamic system with many moving parts, 
    all of which can affect outcomes on the pitch. Limitations are okay.

[^Luck]:

    Luck will also be a factor. A team running hot over multiple seasons will look like meaningful overperformance, and a dip in performances caused by a debilitating injury 
    crisis will, at least partially, be attributed to mistakes made by the club's decision-makers. While the model should, by design, handle this sort of variance well, football 
    is highly random. Some of this randomness will inevitably bleed into some of the model's estimates, and I suspect this will show up most at the club level.
