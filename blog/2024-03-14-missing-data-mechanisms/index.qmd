...
attrition |>
  mutate(
    years_at_company = replace(
      years_at_company,
      runif(n()) < 0.8 & job_satisfaction == "Low",
      NA
    )
  ) |>
  plot_satisfaction() +
...
missing_years <-
  ...
  mutate(
    across(
      starts_with("job") & !job_satisfaction,
      ~ replace(.x, runif(n()) < 0.01, NA)
    ),
    years_at_company = replace(
      years_at_company,
      runif(n()) < 0.75 &
        (job_satisfaction == "Low" | monthly_income > 15000),
      NA
    ),
    relationship_satisfaction = replace(
...
missing_years |>
  ...
  ggplot(aes(
    x = monthly_income,
    fill = forcats::fct_rev(years_at_company_NA)
  )) +
