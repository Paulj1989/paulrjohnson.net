...
#| fig-alt: A bar plot visualising the distribution of Bob's Burgers IMDb user ratings, with a gradient scale distinguishing seasons. The plot demonstrates that earlier seasons tend to have higher ratings, but there are several large outliers in later seasons that have ratings of nine or higher.
#| code-fold: true
...
#| fig-alt: A regression plot visualising the relationship between Bob's Burgers IMDb user ratings and total episode number. While the average rating is just under 8 when the show first begins, it gradually declines over 250+ episodes, to around 7.5.
#| code-fold: true
#| code-summary: 'Plot Code (Click to Expand)'
...
If I were spending more time on this, I would explore any interactions, but I think we can get away with chucking all the potentially valuable features into the mixer here and saving some time.

[^Features]:

I'd highly recommend Emil Hvitfeldt's [_Feature Engineering A-Z_](https://feaz-book.com/), a comprehensive resource on the endless possibilities of feature engineering.
...
I haven't had much experience building custom transformers like this, so I needed a helping hand.
...
For an intuition into random forests (and, by extension, extra trees, too), I'd recommend [MLU-Explain](https://mlu-explain.github.io/random-forest/)'s explainer.

[^GB]:

For an intuition into gradient boosting (and all its variants), I'd recommend Terence Parr & Jeremy Howard's [_How to Explain Gradient Boosting_](https://explained.ai/gradient-boosting/)

[^Optuna]:

There are other hyperparameter optimisation libraries, such as HyerOpt, and scikit-learn, which offers tuning functionality of its own, but Optuna is the most [feature-rich](https://neptune.ai/blog/optuna-vs-hyperopt). Perhaps the focus should be on [methods, not libraries](https://trainindata.substack.com/p/choose-the-library-based-on-the-method), but in my experience, Optuna's methods also provide excellent performance.
...
A validation set or a more involved process like nested cross-validation would add some value. I don't think we have enough data for a validation set, and I decided a fully nested pipeline was overkill, but I'm sure it would be beneficial.
...
Some simple preprocessing steps could improve the linear regression, potentially making it competitive with our final model. If I intended to put this into production, I'd be tempted to run the linear regression alongside the random forest to see if a simpler model would ultimately come out on top.
...
```{python}
#| label: error-dist
#| fig-alt: A histogram visualising the distribution of prediction errors for the random forest predicting Bob's Burgers IMDb user ratings. While the majority of errors appear to be positive and the distribution peaks just above zero, the negative errors have a longer tail.
#| code-fold: true
#| code-summary: 'Plot Code (Click to Expand)'
...
#| fig-alt: A regression plot visualising the relationship between predicted and actual ratings for the random forest model predicting Bob's Burgers IMDb user ratings. On average, pedictions seem to overrate episode ratings.
#| code-fold: true
#| code-summary: 'Plot Code (Click to Expand)'
#| cache: true
...
#| fig-alt: A regression plot visualising the relationship between IMDb ratings and the absolute prediction error for the random forest model predicting Bob's Burgers ratings. While the model performs relatively well, it struggles at the extreme ends of the ratings.
#| code-fold: true
#| code-summary: 'Plot Code (Click to Expand)'
