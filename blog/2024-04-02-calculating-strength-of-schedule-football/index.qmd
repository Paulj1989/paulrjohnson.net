...
strength_of_schedule <-
  ...
  left_join(opponent_opponent_ratings, by = join_by(team)) |>
  mutate(
    unstandardized = (2 / 3 * opponent_rating) +
      (1 / 3 * opponent_opponent_rating),
...
strength_of_schedule |>
  ...
  gt() |>
  tab_footnote(
    footnote = "z < 0 = Easier than league average; z > 0 = Harder than league average"
  ) |>
  tbl_theme(type = "num")
```
...
strength_of_schedule |>
  ...
  labs(
    ...
    x = NULL,
    y = "Strength of Schedule (Z-Score)",
...
champ_sos |>
  filter(
    team %in% c("Ipswich Town", "Leeds United", "Leicester City", "Southampton")
  ) |>
  arrange(sos) |>
  rename(
    "Team" = team,
